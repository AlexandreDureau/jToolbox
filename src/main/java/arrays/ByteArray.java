...
public class ByteArray {
    ...
    public static byte[] s_removeAt(byte[] buffer, int index) {

        if(index >=0 && index < buffer.length){
            byte[] result_buffer = new byte[buffer.length - 1];

            if(index == 0){
                System.arraycopy(buffer, index+1, result_buffer, 0, buffer.length - 1);
            }
            else if(index == buffer.length-1){
                System.arraycopy(buffer, 0, result_buffer, 0, buffer.length - 1);
            }
            else{
                System.arraycopy(buffer, 0, result_buffer, 0, index);
                System.arraycopy(buffer, index+1, result_buffer, index, buffer.length - index - 1);
            }
    ...
    /**
     * Remove from a buffer the element at given index
     *
     * @param buffer the buffer
    ...
    public byte removeAt(int index) {
        ...
        byte byteElement = byteArray[index];
        byteArray = s_removeAt(byteArray, index);
